...
Dadurch kann es allerdings dazu kommen, dass das Chromosom vor und nach der Mutation zwar unterschiedliche Genotypen aufweisen, sich die aktiven Knoten allerdings nicht voneinander unterscheiden.
Um dieses Problem in den Griff zu bekommen, wird in dieser Arbeit die Single (Active) Mutation herangezogen.
In diesem Algorithmus werden zufällig die Gene eines Chromosoms verändert, bis ein aktiver Knoten mutiert wurde.
...
TODO: für welche verschiedene Knotentypen können welche Dinge geändert werden

Um die Single Active Mutation anhand eines Beispiels näher zu erläutern wird folgender Gentyp eines Chromosoms aus Abbildung \ref{fig:genotyp} erneut eingeführt.
Die Einträge wurden teilweise angepasst, sodass mehr inaktive Knoten zur Verfügung stehen.
Zur Veranschaulichung des Prozesses wurden die beiden Inputs mit den Indices 0 und 1 ebenfalls in die Abbildung eingefügt.
Da die Einträge der Input-Gene keine relevanten Informationen enthalten, werden diese durch ein \glqq -\grqq\space angegeben.
...
\begin{figure}[H]
	\centering
	\includegraphics[scale = 0.6]{Bilder/Genotyp.png}
	\caption{Beispiel Genotyp vor Mutation, angelehnt an \cite{torabi_using_2022}}
	\label{fig:genotypVorMutation}
\end{figure}

TODO: Abbildung anpassen, sodass weniger aktive Knoten da sind (am besten ein Input ohne aktiv)

Wie anhand des Genotyps erkannt werden kann, sind die Gene mit den folgenden Indices aktive Knoten:
TODO aktive Knoten
Der Algorithmus beginnt und ein zufälliger Index wird für die Mutation ausgewählt.
Angenommen dieser Index hat den Wert TODO

TODO: Weitere Durchgänge mit den unterschiedlichen Knotentypen
