...
MW / PL-Platzierungen:
Parity:
- 16 mal mit Rekombination besser; 2 mal ohne 
=> Rekombination kann Effizienz steigern bei sinnvoller Konfiguration
- One-Point Rekombination bessere Ergebnisse als andere Rekombinationsarten
=> weniger Einfluss von Rekombination (kleine Schrittweiten) scheinen von Vorteil zu sein (vermutlich weil Rekombination und Mutation nicht aneinander abgestimmt sind)
=> Anpassen der Verfahren könnte Potential aus komplexeren Rekombinationsarten besser ausschöpfen

Keijzer:
- 12 aus 18 mal mit Rekombination besser als ohne
- alle Mittelwerte je Rekombinationsrate kleiner als bei "keine Rekombination"
- im Standard-CGP-Verfahren lohnt sich Rekombination im Durchschnitt
- One-Point im Durchschnitt am besten (nach MW)
- beste und schlechteste Ergebnisse innerhalb von einer Rekombinationsart (hier Two-Point) => richtige Konfiguration ist relevant
- schlechte MW und PL Ergebnisse bei Konfigurationen mit viel negativen Mutationen
=> hier haben Rekombination und Mutation wahrscheinlich am meisten gegeneinander gearbeitet
- beste Ergebnisse bei Konfigurationen mit entweder wenig pos. Rekombinationen oder anteilig wenig negativer Mutation (gegenüber pos. Rekombination)
=> wahrscheinlich die Verfahren wo Rekombination weniger Einfluss hat oder sich Rekombination und Mutation weniger schlecht beeinflussen

Encode:
- One-Point und Uniform: 20 aus 22 Verfahren mit Rekombination besser als ohne
- Two-Point: mit Rekombination immer schlechter
- Uniform bietet für jede Rekombinationsraten-Art beste Ergebnisse
- One-Point: one-fifth am besten, dann konstante, dann clegg
- Two-Point: One-fifth, clegg, konstant
- Uniform: Clegg, One-Fifth, Konstant
=> nicht "die beste" Rekombinationsraten-Art feststellbar
=> mit HPO verschiedene testen ist sinnvoll

Koza:
- One-Point: kein Verfahren besser mit Rekombination als ohne
- Two-Point: 5 aus 22 besser mit Rekombination
- Uniform: 3 aus 22 besser mit Rekombination
- Rekombination lohnt sich bei Standard-Verfahren erstmal nicht (aber nicht vergessen, dass keine HPO ausgeführt wurde)
=> mögliche Folgerung: Hyperparameter müssen alle gleichzeitig optimiert werden
=> andere mögliche Folgerung: für Testfälle, die dann länger brauchen wird Rekombination zur Last, weil in späteren Iterationen die Mutation wichtiger wird (Uniform mit Clegg als beste Lösung könnte das untermauern)
- One-Point: für alle Rekombinationsraten-Arten schlecht
- Two-Point: für alle Rekombinationsraten-Arten relativ gut
- Uniform: nur für Clegg gut
=> für lange Trainings macht es wahrscheinlich eher Sinn Rekombination zu cutten 
=> könnte Grund sein, warum Clegg für Parity und Keijzer nicht besser abschließt: Rekombinationsrate müsste schneller fallen
- komplexere Rekombinationsarten geben für Clegg bessere Ergebnisse => könnte an höherem Einfluss liegen (mehr pos. Rekombinationsraten)
=> wenn Rekombination rechtzeitig komplett abgestellt wird, sind wahrscheinlich komplexere Rekombinationsarten sinnvoll



Offset:
Parity:
- 6 aus 9 Verfahren ohne Offset besser als mit
- Offset schadet eher 
=> wahrscheinlich werden positive Effekte aus den ersten Iterationen gestrichen während negative Effekte aus späteren Iterationen erhalten bleiben

Keijzer:
- 5 / 6 aus 9 ohne Offset besser als mit
=> im Durchschnitt ist es besser Rekombination direkt auszuführen
=> wahrscheinlich werden positive Effekte aus den ersten Iterationen gestrichen während negative Effekte aus späteren Iterationen erhalten bleiben
- Ergebnisse zeigen, dass Verfahren noch verbessert werden muss, damit Offset in keinen Verfahren mehr Sinn macht

HPDI:
Parity:
- One-Point beste (kleine) Ranges
- kleine untere Grenzen führen zu weniger Umfang
=> frühe Trainingserfolge besonders wichtig (frühe Konvergenzen führen zu weniger Streuung)
=> Anpassen des Verfahren für bessere Rekombination ist wichtig

Keijzer:
- Uniform relativ wenig Umfänge
- One-Point zwei geringe und sonst große Umfänge
=> anders als bei Parity => Umfänge variieren für Rekombinationsarten => keine allgemeine Aussage möglich
- Beobachtungen mit unteren Grenzen hier genauso
- mit oberen Grenzen genau das gleiche

Encode:
- Beobachtungen von Parity und Keijzer gelten nur für Iterationen und nicht für Fitness bei einer Iteration

Koza:
- anderes Verfahren als bei Parity und Keijzer (umgekehrt): zuerst alle kleinen und großen Grenzen markiert und dann geschaut ob die matchen
=> gleiche Aussage wie bei Parity und Koza
=> + zusätzlich: Streuungen früh einschätzen möglich je nach Iterationen bei Konvergenz der Durchläufe => bei niedrigen Iterationen bleiben wahrscheinlich alle niedrig => bei hohen Iterationen werden die anderen Wahrscheinlich noch höher



- Notizen zu MW: Parity / Keijzer bevorzugen One-Point und Encode/Koza nicht => woran liegt das => Parity und Keijzer sind optimiert => das heißt aus One-Point kann mit HPO am meisten herausgeholt werden mit jetzigem Verfahren (evtl wegen niedrigem Einfluss in Lernen; kleinere Schritte im Lösungsraum / kleinschrittigere Veränderungen lassen sich mit Mutation wahrscheinlich besser in Einklang bringen) => Rohdatenanalyse zeigt, dass Uniform mehr Potential besitzt, also Verfahren verbessern kann Uniform evtl noch pushen
