...
def calvo_and_hpdi_for_all():
    ...
    for i in range(0,23,1):
        data = np.load(f"BayesianAnalysis/helperForLinuxVM/kozaTwoPointData/data{i}.npz")
...
def calvo(algorithm_labels, results):
    ...
    with open("BayesianAnalysis/bayesianResults/KozaTwoPoint/calvo.txt", "a") as file:
        file.write(f"{sample}")
...
def hpdi(result, max_iteration_without_censoring, number_censored, name_algo):
    ...
    with open("BayesianAnalysis/bayesianResults/KozaTwoPoint/hpdi.txt", "a") as file:
        file.write(f"{name_algo} & {hdi} & {mean} \\\\\n\\hline\n")



with open("BayesianAnalysis/bayesianResults/KozaTwoPoint/hpdi.txt", "w") as file:
    file.write("")


with open("BayesianAnalysis/bayesianResults/KozaTwoPoint/calvo.txt", "w") as file:
    file.write("")
