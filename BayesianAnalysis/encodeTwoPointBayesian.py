...
def calvo_and_hpdi_for_all():
    ...
    for i in range(0,23,1):
        data = np.load(f"BayesianAnalysis/helperForLinuxVM/encodeTwoPointData/data{i}.npz")
...
def calvo(algorithm_labels, results):
    ...
    with open("BayesianAnalysis/bayesianResults/EncodeTwoPoint/calvo.txt", "a") as file:
        file.write(f"{sample}")


def hpdi(result, name_algo):
    #  ++++++++++++++++++++++++++++++++++++++
    # HPDI (& Ranking)
    #  ++++++++++++++++++++++++++++++++++++++
    ...
    data = np.sort(data)
    hdi = (round(data[int(len(data) * 0.025)],5), round(data[int(len(data) * 0.975)], 5))

    mean = round(np.mean(data),5)
    print("HDPI Algo 2: ", hdi)


    with open("BayesianAnalysis/bayesianResults/EncodeTwoPoint/hpdi.txt", "a") as file:
        file.write(f"{name_algo} & {hdi} & {mean}\\\\\n\\hline\n")



with open("BayesianAnalysis/bayesianResults/EncodeTwoPoint/hpdi.txt", "w") as file:
    file.write("")


with open("BayesianAnalysis/bayesianResults/EncodeTwoPoint/calvo.txt", "w") as file:
    file.write("")
