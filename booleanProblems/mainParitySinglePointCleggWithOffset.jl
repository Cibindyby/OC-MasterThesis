...
function writeHpoResults(results::String)
    ...
    save_path = joinpath(["Experiments_Boolean", 
                            ...
                            "Crossover Rate Type 2 with offset",
                            "HPOResults.txt"])
...
function hpo(iteration::Int)
    ho = @hyperopt for i = 40000, #aus 400000 Kombinationen
        ...
        crossover_delta = 0.001:0.005:0.051, #angepasst an ca 20 Generationen, in guten Fällen bis 1000 Generationen in schlechten
        elitism_number = 2:2:20,
        crossover_offset = 20:50:520
        
        meanAusMehrerenIterationen(nbr_computational_nodes, population_size, crossover_delta, elitism_number, crossover_offset)
    end
    ...
    writeHpoResults("Iteration $iteration: Parameter -> $beste_parameter; Ergebnis -> $bestes_ergebnis (nbr_computational_nodes, population_size, crossover_delta, elitism_number, crossover_offset)")
end

function meanAusMehrerenIterationen(nbr_computational_nodes, population_size, crossover_delta, elitism_number, crossover_offset)
