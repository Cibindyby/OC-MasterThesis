...
function main()
    ...
    while iterations < eval_after_iterations
        ...
        if get_best_fitness(runner) < 0.01
            break
        end
...
#TODO: Problem lösen, dass für jeden Durchgang eine neue CSV Datei beschrieben werden soll
...
crossover_start = worksheet[lineCounter,:][6]
crossover_delta = worksheet[lineCounter,:][7]
elitism_number = worksheet[lineCounter,:][8]
crossover_offset = worksheet[lineCounter,:][9]

mu = 1
lambda = 4
eval_after_iterations = 6500
nbr_inputs = 1 #Achtung, das gilt nur für Parity!!!
nbr_outputs = 1
tournament_size = 0
