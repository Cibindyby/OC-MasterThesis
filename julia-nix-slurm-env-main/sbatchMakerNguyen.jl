...
srun nix develop --impure --command bash -c \"
    just justfilefunc /data/oc-compute03/ebertzci/OC-MasterThesis/symbolicRegression/nguyenMain.jl $testproblem $c_art $c_r_art $offset $c_r_value $wert_offset \$SLURM_ARRAY_TASK_ID
\"
            "
            ...
            if c_art == 3
                save_path = joinpath(["scripts",
                                        "nguyenNoCrossover.sbatch"])
                mkpath(dirname(save_path))

            else

                save_path = joinpath(["scripts",
                                            "nguyen$c_art$(c_r_art)CRV$nachkommastellen_crossover_rate.sbatch"])
                mkpath(dirname(save_path))
            end
